 ...
 * - address: Token contract address (0x-prefixed hex or Solana base58)
 * - chain: Chain. Can be `ethereum`, `base`, `solana`, or `tron`.
 ...
 * - offset?: Pagination offset (default: 0) @min 0
 */
export async function fetchTokenTransfers(params: { address: string; chain: 'ethereum' | 'base' | 'solana' | 'tron'; from?: string; to?: string; limit?: number; offset?: number }) {
  if (params.limit !== undefined) params.limit = Math.max(1, Math.min(100, params.limit))
  if (params.offset !== undefined) params.offset = Math.max(0, params.offset)
  const qs: Record<string, string> = {}
  qs['address'] = String(params.address)
...
// ---------------------------------------------------------------------------
// Token infinite query hooks (auto-pagination)
// ---------------------------------------------------------------------------
...
/** Get token DEX trade history — infinite-scroll wrapper with automatic offset pagination. */
export function useInfiniteTokenDexTrades(params: Omit<Parameters<typeof fetchTokenDexTrades>[0], 'offset'>, opts?: OffsetInfiniteOpts<ApiResponse<DexTradeItem>>) {
  return useInfiniteQuery({
    queryKey: ['token', 'dex', 'trades', 'infinite', params],
    queryFn: ({ pageParam = 0 }) => fetchTokenDexTrades({ ...params!, offset: pageParam }),
    initialPageParam: 0,
...
/** Get token holders — infinite-scroll wrapper with automatic offset pagination. */
export function useInfiniteTokenHolders(params: Omit<Parameters<typeof fetchTokenHolders>[0], 'offset'>, opts?: OffsetInfiniteOpts<ApiResponse<TokenHolderItem>>) {
  return useInfiniteQuery({
    queryKey: ['token', 'holders', 'infinite', params],
    queryFn: ({ pageParam = 0 }) => fetchTokenHolders({ ...params!, offset: pageParam }),
    initialPageParam: 0,
    getNextPageParam: (lastPage) => {
      const meta = (lastPage as any)?.meta
      if (!meta?.total || !meta?.limit) return undefined
...
/** Get token transfers — infinite-scroll wrapper with automatic offset pagination. */
export function useInfiniteTokenTransfers(params: Omit<Parameters<typeof fetchTokenTransfers>[0], 'offset'>, opts?: OffsetInfiniteOpts<ApiResponse<TokenTransferItem>>) {
  return useInfiniteQuery({
    queryKey: ['token', 'transfers', 'infinite', params],
    queryFn: ({ pageParam = 0 }) => fetchTokenTransfers({ ...params!, offset: pageParam }),
    initialPageParam: 0,
    getNextPageParam: (lastPage) => {
      const meta = (lastPage as any)?.meta
      if (!meta?.total || !meta?.limit) return undefined
