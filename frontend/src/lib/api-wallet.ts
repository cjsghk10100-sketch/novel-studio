...
// ---------------------------------------------------------------------------
// Wallet infinite query hooks (auto-pagination)
// ---------------------------------------------------------------------------
...
/** Get wallet transaction history — infinite-scroll wrapper with automatic offset pagination. */
export function useInfiniteWalletHistory(params: Omit<Parameters<typeof fetchWalletHistory>[0], 'offset'>, opts?: OffsetInfiniteOpts<ApiResponse<WalletHistoryItem>>) {
  return useInfiniteQuery({
    queryKey: ['wallet', 'history', 'infinite', params],
    queryFn: ({ pageParam = 0 }) => fetchWalletHistory({ ...params!, offset: pageParam }),
    initialPageParam: 0,
...
/** Get wallet DeFi protocol positions — infinite-scroll wrapper with automatic offset pagination. */
export function useInfiniteWalletProtocols(params: Omit<Parameters<typeof fetchWalletProtocols>[0], 'offset'>, opts?: OffsetInfiniteOpts<ApiResponse<WalletProtocolItem>>) {
  return useInfiniteQuery({
    queryKey: ['wallet', 'protocols', 'infinite', params],
    queryFn: ({ pageParam = 0 }) => fetchWalletProtocols({ ...params!, offset: pageParam }),
    initialPageParam: 0,
    getNextPageParam: (lastPage) => {
      const meta = (lastPage as any)?.meta
      if (!meta?.total || !meta?.limit) return undefined
...
/** Get wallet transfers — infinite-scroll wrapper with automatic offset pagination. */
export function useInfiniteWalletTransfers(params: Omit<Parameters<typeof fetchWalletTransfers>[0], 'offset'>, opts?: OffsetInfiniteOpts<ApiResponse<WalletTransferItem>>) {
  return useInfiniteQuery({
    queryKey: ['wallet', 'transfers', 'infinite', params],
    queryFn: ({ pageParam = 0 }) => fetchWalletTransfers({ ...params!, offset: pageParam }),
    initialPageParam: 0,
    getNextPageParam: (lastPage) => {
      const meta = (lastPage as any)?.meta
      if (!meta?.total || !meta?.limit) return undefined
